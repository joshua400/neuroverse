...
import type { MemoryRecord, MemoryQuery, MemoryTier } from "../types.js";
...
const shortTerm = new Map<string, MemoryRecord[]>();
...
function loadPersistent(): MemoryRecord[] {
  const p = persistPath();
  if (!existsSync(p)) return [];
  try {
    return JSON.parse(readFileSync(p, "utf-8")) as MemoryRecord[];
  } catch {
    return [];
  }
}

function savePersistent(records: MemoryRecord[]): void {
