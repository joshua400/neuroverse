...
export function configureVoice(cfg: Partial<VoiceConfig>) {
  config = { ...config, ...cfg };
}
...
export async function transcribeAudio(audioPath: string): Promise<string> {
  if (!config.whisperEndpoint) throw new Error("Whisper endpoint not configured.");
  ...
  try {
    const response = await axios.post(
      ...
      { file_path: audioPath }, // Adjust based on your Whisper REST API spec
