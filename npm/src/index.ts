...
import { extractIntent, ruleBasedExtract } from "./core/intent.js";
...
import { TaskType, MemoryTier } from "./types.js";
...
Returns:
  ...
  async (params) => {
    const taskType = params.task_type as TaskType;
    const decision = routeTask(taskType);

    const result: Record<string, unknown> = { routing: decision };
